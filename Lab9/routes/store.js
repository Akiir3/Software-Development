 ...
 app.get('/', function(req, res, next) {
	 ...
	 var sqlQuery = 'select * from lab6_1.store';
...
if( !errors ) { //No errors were found. Passed Validation!
 ...
 values for SET*/
 conn.query('INSERT INTO lab6_1.store SET ?', item,
 function(err, result) {
 	if (err) {
	 req.flash('error', err)
	 // render to views/store/add.ejs
...
res.render('store/add', {
title: 'Add New Item', sname: req.body.sname,
qty: req.body.qty,
...
app.get('/edit/(:id)', function(req, res, next) {
    ...
    */
    ...
    req.getConnection(function(error, conn) {
        conn.query('SELECT * FROM lab6_1.store WHERE id = ' + req.params.id + ";", 
	function(err, rows,
            fields) {
            // if item not found
            if (rows.length <= 0) {
...
conn.query( 'UPDATE lab6_1.store SET sname = "' + req.body.sname 
	+ '", qty = ' + req.body.qty + ", price = " + 				
...
app.delete('/delete/(:id)', function(req, res, next) {
    req.getConnection(function(error, conn) {
        conn.query('DELETE FROM lab6_1.store where id = ' + req.params.id + ";", 
	function(err, result) {
            if (err) {
                req.flash('error', err)
                ...
                req.flash('success', 'Success, you have deleted: ' + 			
		req.params.id)
                // redirect to users list page
                res.redirect('/store')
            }
